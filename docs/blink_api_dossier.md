...
**E14 — Authenticated client conditionally adds Authorization and TOKEN-AUTH on Blink hosts**
- File: `logs/blink-apk/57.1-29715642/decompiled/jadx/sources/com/immediasemi/blink/inject/NetworkModule.java`
```java
...
**E78 — OAuth env persistence, lookup, and subdomain fallback**
- File: `logs/blink-apk/57.1-29715642/decompiled/jadx/sources/com/immediasemi/blink/common/network/tier/TierRepository.java`
```java
if (TierRepository.this.persistentClientPreferences.put(
    TierRepository.OAUTH_ENV_KEY, this.$tier.getOauthEnvironment(), this)
    == coroutine_suspended) { ... }
```
```java
...
**E83 — TierInfo persistence writes account_id, then tier**
- File: `logs/blink-apk/57.1-29715642/decompiled/jadx/sources/com/immediasemi/blink/common/network/tier/TierRepository.java`
```java
...
...
if (TierRepository.this.accountPreferences.put(TierRepository.TIER_KEY,
    this.$tierInfo.getTier(), this) == coroutine_suspended) { ... }
```
The second write is exposed by JADX's recovered missing-block comment and is
independently present as `TierInfo.getTier()` followed by a `"TIER"` preference
write in
`logs/blink-apk/57.1-29715642/decompiled/apktool-base/smali_classes11/com/immediasemi/blink/common/network/tier/TierRepository$setTierInfo$2.smali`.

**E84 — AccountRepository later re-persists account tier**
...
**E93 — OAuth persistence does not supply an account tier or region**

- File: `logs/blink-apk/57.1-29715642/decompiled/jadx/sources/com/immediasemi/blink/common/account/auth/AuthorizationRepository$exchangeAuthorizationToken$2.java`

```java
TokenResponse tokenResponse = (TokenResponse) obj;
String accessToken = tokenResponse.accessToken;
String refreshToken = tokenResponse.refreshToken;
credentialRepository.setTokens(accessToken, refreshToken, ...);
```

- File: `logs/blink-apk/57.1-29715642/decompiled/jadx/sources/com/immediasemi/blink/common/account/auth/RefreshTokensResponse.java`
...
The authorization repository persists only AppAuth's access and refresh tokens,
and the APK refresh DTO has no account tier or region property. Region routing
therefore cannot be derived from either token response; it comes from
`TierRepository` state and the later `tier_info` response.

**E94 — Native Android AppAuth hosted-page proof (bounded live result)**

A git-ignored, secret-blind Java harness reproduced Android 57.1's AppAuth
authorization request, exact App-Link, manufacturer-derived client choice, and
same-device token callback handling. On an Android 36 emulator with the official
Blink 57.1 split APK installed, the exact native request reached the production
Blink hosted identity document. Five authorize variations also reached hosted
sign-in. Deliberately invalid token forms reached the production token parser
and returned the bounded `invalid_grant` category. No owner credential, MFA,
callback, state, verifier, code, or token value was recorded. This proves
native hosted-page launch and endpoint construction, not a fresh owner-account
code exchange.

**E95 — EU Android refresh and production-default gateway proof (bounded live result)**

Using only the authorized EU/Ireland account, a still-valid legacy refresh
session was accepted by the exact Android refresh form and rotated. The
resulting Android-profile state refreshed again through the installed plugin,
reached user information and homescreen/device discovery, and survived two
clean Homebridge restarts with owner-only state and no credentials in config.
A read-only `tier_info` probe using that Android state returned HTTP 406 from
`prod` and `a001`, and HTTP 200 plus the same valid authoritative tier from
`prde` and `prsg`. No account identifier, token, email, device name, serial, or
network address was retained in this dossier.
...
| OAuth-persisted values | `AuthorizationRepository` / `RefreshTokensResponse` | tokens/expiry/scope/type; no account tier or region |
...
5. **Tier bootstrap:** the token response contains no account region or tier.
   Native Android calls `v1/users/tier_info` through its already-persisted
   default tier. Homebridge has no Android registration-region preference, so
   it tries the ordinary APK production defaults `prod`, `prde`, `prsg`, and
   `a001` in that order, advancing only after HTTP 406. It validates the first
   successful response with the APK's four-alphanumeric rule and rebuilds
   REST/shared REST bases before account information and homescreen requests.
...
values. A later native AppAuth harness reproduced the exact Android request and
reached Blink's hosted sign-in page. These prove the hosted native surface, not
a fresh packaged `0.9.1` Homebridge code exchange.

Two later packaged-flow callbacks reached the production token endpoint and
returned `BHO-HTTP-INVALID-GRANT`. Separately, Blink accepted the authorized
account's valid legacy refresh session with the exact Android cross-client form.
The resulting Android-profile state rotated again through the installed plugin,
reached user information and homescreen/device discovery, and survived two
clean restarts. That proves the Android refresh/runtime path, not a new hosted
authorization-code exchange.

A live Android-token gateway probe returned HTTP 406 from `prod` and `a001`, and
HTTP 200 with a valid authoritative tier from `prde` and `prsg`. This is account
reachability evidence for the EU token only; it is not live validation of US,
AP, or AU accounts. `cemp` and `srf1` remain explicit APK targets but are
regression/refurbishment targets and are not ordinary account bootstrap
candidates. Numbered four-character e-tiers are accepted because
...
tier; only post-token REST/shared-REST routing changes. Fresh packaged
...
- EU, US, AP, and AU are production account tiers, not OAuth environments; all
  therefore use the same production OAuth host. OAuth responses have no account
  tier or region; a current/default REST tier is needed before `tier_info`
  returns the authoritative destination. (E93)
...
- `setDefaultTier` persists `OAUTH_ENV` in client preferences; unknown values
  fall back to PRODUCTION, whose subdomain is empty. (E78, E8)
...
- Login fetches `tier_info` and calls `setTierInfo`, which immediately persists
  both `account_id` and `tier`. The next `AccountApi.getAccountInfo` call is
  therefore already rewritten to the discovered regional host;
  `AccountRepository.setAccount()` later re-persists `Account.tier`. (E82,
  E83, E84, E86, E87)
- Homebridge hosted completion lacks Android's persistent registration-region
  selection. It probes only `prod`, `prde`, `prsg`, and `a001`, advances only on
  HTTP 406, and stops at the first valid service tier. It never probes `cemp` or
  `srf1` for an ordinary account. EU live evidence and non-EU mocked coverage are
  recorded in E95.
...
- `OauthApi.postLogin` still exposes a direct credential method inside the APK
  ...
  plugin's current UI and not part of the `0.9.1` operator workflow.
- The plugin runtime separately retains a hidden, deprecated compatibility
  fallback for paired manual `username` and `password` configuration. It can
  execute legacy iOS-profile credential sign-in when compatible state is
  absent or unusable; unlike the supported hosted UI, that path gives the
  credentials to Homebridge.
...
- The authorized EU account also accepted a legacy refresh token with that
  Android form and produced working Android-profile state. This bounded
  cross-client migration result is not treated as a Blink guarantee and is not
  applied automatically to every legacy installation. (E95)
...
The APK's general authenticated Retrofit client adds bearer when an access
token exists and adds `TOKEN-AUTH` only when registration-token state exists
(E14). The Homebridge hosted-Android profile uses bearer alone, matching the
live Ireland proof; legacy persisted profiles can retain `TOKEN-AUTH` when
present. Endpoint Auth cells below use that conditional general-client
contract.

| Method | Base Host | Path | Purpose (method) | Auth | Headers | Body Schema | Response Shape | Evidence |
|---|---|---|---|---|---|---|---|---|
| DELETE | https://rest-{tier}.immedia-semi.com/api/ | `v1/shared/invitations/{invitationId}/decline` | `AccessApi.m16017deleteDeclineInvitegIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E32 |
| DELETE | https://rest-{tier}.immedia-semi.com/api/ | `v1/shared/authorizations/{authorizationId}/remove` | `AccessApi.m16018deleteRemoveAccessgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<PollingResponse> | E32 |
| DELETE | https://rest-{tier}.immedia-semi.com/api/ | `v1/shared/authorizations/{authorizationId}/revoke` | `AccessApi.m16019deleteRevokeAccessgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E32 |
| DELETE | https://rest-{tier}.immedia-semi.com/api/ | `v1/shared/invitations/{invitationId}/revoke` | `AccessApi.m16020deleteRevokeInvitegIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E32 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/shared/check_authorization` | `AccessApi.m16021getCheckAuthorizationgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CheckAuthorizationResponse> | E32 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/shared/summary` | `AccessApi.m16022getSharedSummaryIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<AccessSummary> | E32 |
| PATCH | https://rest-{tier}.immedia-semi.com/api/ | `v1/shared/authorizations/{authorizationId}` | `AccessApi.m16023patchFriendlyNameBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | FriendlyNamePatchBody | Result<PollingResponse> | E32 |
| PATCH | https://rest-{tier}.immedia-semi.com/api/ | `v1/shared/popovers/{popoverId}/read` | `AccessApi.m16024popoverReadgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E32 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/shared/invitations/{invitationId}/accept` | `AccessApi.m16025postAcceptAccess0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AcceptInvitationBody | Result<PollingResponse> | E32 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/shared/invitations/send` | `AccessApi.m16026postSendInvitegIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | SendInviteBody | Result<Unit> | E32 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/accessories/add` | `AccessoryApi.m16411addAccessory0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AddAccessoryBody | Result<Kommand> | E33 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/accessories/delete` | `AccessoryApi.m16412delete0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | DeleteAccessoryBody | Result<Kommand> | E33 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/users/authenticate_password` | `AccountApi.m16037authenticatePasswordgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AuthenticatePasswordBody | Result<AuthenticatePasswordResponse> | E34 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `/users/delete` | `AccountApi.m16038deleteAccountgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | DeleteAccountBody | Result<Unit> | E34 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v2/users/info` | `AccountApi.m16039getAccountInfoIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Account> | E34 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/users/options` | `AccountApi.m16040getAccountOptionsIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<AccountOptionsResponse> | E34 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/users/preferences` | `AccountApi.m16041getAccountPreferencesIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<AccountPreferencesBody> | E34 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/notifications/preferences` | `AccountApi.m16042getNotificationPreferencesIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<NotificationPreferencesResponse> | E34 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/users/tier_info` | `AccountApi.m16043getTierInfoIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<TierInfo> | E34 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/clients/%7Binjected_client_id%7D/logout` | `AccountApi.m16044logoutIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E34 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/users/preferences` | `AccountApi.m16045postAccountPreferencesgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AccountPreferencesBody | Result<AccountPreferencesBody> | E34 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/notifications/preferences` | `AccountApi.m16046postNotificationPreferencesgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | NotificationPreferencesResponse | Result<Unit> | E34 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/users/pin/resend` | `AccountApi.m16047postRegistrationPinResendIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<GeneratePinResponse> | E34 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/users/pin/verify` | `AccountApi.m16048postRegistrationPinVerifygIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | VerifyPinPostBody | Result<VerifyPinResponse> | E34 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/identity/token` | `AccountApi.m16049postTokenUpgradegIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | TokenUpgradePostBody | Result<RefreshTokensResponse> | E34 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/countries/update` | `AccountApi.m16050setAccountCountrygIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | CountryBody | Result<CountryResponse> | E34 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/users/countries/update` | `AccountApi.m16051updateUserCountrygIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | CountryBody | Result<CountryResponse> | E34 |
| DELETE | https://rest-{tier}.immedia-semi.com/api/ | `v1/alexa/link` | `AlexaLinkingApi.m17513deleteLinkIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E35 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/alexa/link_status` | `AlexaLinkingApi.m17514getLinkStatusIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<AlexaLinkStatus> | E35 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/alexa/authorization` | `AlexaLinkingApi.m17515postAuthorizationgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AlexaLinkingAuthorizePostBody | Result<AlexaLinkingAuthorizeResponse> | E35 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/alexa/link` | `AlexaLinkingApi.m17516postLinkgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AlexaLinkingLinkPostBody | Result<Unit> | E35 |
...
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/add` | `CameraApi.m16110addCamera0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AddCameraBody | Result<AddCameraResponseBody> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/cameras/{camera}/accessories/{accessoryType}/{accessoryId}/delete` | `CameraApi.m16111deleteAccessoryyxL6bBk` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{networkId}/cameras/{cameraId}/delete` | `CameraApi.m16112deleteCamera0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E37 |
| DELETE | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{primary_id}/pair` | `CameraApi.m16113deleteCameraPair0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/delete` | `CameraApi.deleteCameraRx` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<BlinkData> | E37 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/config` | `CameraApi.m16114getCameraConfig0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CameraConfig> | E37 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/config` | `CameraApi.getCameraConfigRx` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<CameraConfig> | E37 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/cameras/{cameraId}/network_type` | `CameraApi.m16115getVideoNetworkType0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<VideoNetworksConfig> | E37 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/zones` | `CameraApi.getZones` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<AdvancedCameraZones> | E37 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/zones` | `CameraApi.m16116getZonesV20E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<ZoneV2Response> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{networkId}/cameras/{camera}/light_accessories/{accessoryId}/lights/{lightControl}` | `CameraApi.m16117postAccessoryLightyxL6bBk` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CameraActionKommand> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{networkId}/cameras/{cameraId}/config` | `CameraApi.m16118postCameraConfigBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateCameraBody | Result<Kommand> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/{type}` | `CameraApi.m16119postCameraMotionBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CameraActionKommand> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/{type}` | `CameraApi.postCameraMotionOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<Command> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{networkId}/cameras/{camera}/config` | `CameraApi.postCameraSettingsRx` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateCameraBody | Observable<Command> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{networkId}/cameras/{cameraId}/status` | `CameraApi.m16120postCameraStatusCommand0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v6/accounts/%7Binjected_account_id%7D/networks/{networkId}/cameras/{cameraId}/liveview` | `CameraApi.m16121postLiveViewCommandBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | LiveViewCommandPostBody | Result<LiveViewCommandResponse> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{primary_id}/pair` | `CameraApi.m16122postPairCamerasBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | PairCameraBody | Result<Unit> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{primary_id}/swap_pair` | `CameraApi.m16123postSwapPairBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | SwapCameraBody | Result<Unit> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/thumbnail` | `CameraApi.m16124postThumbnail0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CameraActionKommand> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/thumbnail` | `CameraApi.postThumbnailOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<Command> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/cameras/{cameraId}/network_type` | `CameraApi.m16125postVideoNetworkTypeBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | VideoNetworkTypeBody | Result<Kommand> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/calibrate` | `CameraApi.m16126saveCalibrateTemperatureBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | TemperatureCalibrationPostBody | Result<Kommand> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/zones` | `CameraApi.setZones` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AdvancedCameraZones | Observable<Command> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/zones` | `CameraApi.m16127setZonesV2BWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | ZoneV2Response | Result<Kommand> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/cameras/{camera_id}/snooze` | `CameraApi.m16128snoozeCameraBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | SnoozeBody | Result<Unit> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/temp_alert_disable` | `CameraApi.m16129tempAlertDisable0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/cameras/{camera}/temp_alert_enable` | `CameraApi.m16130tempAlertEnable0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E37 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/cameras/{camera_id}/unsnooze` | `CameraApi.m16131unSnoozeCamera0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E37 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/clients/%7Binjected_client_id%7D/options` | `ClientApi.m16065getClientOptionsIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<ClientOptionsBody> | E38 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/clients/%7Binjected_client_id%7D/options` | `ClientApi.m16066postClientOptionsgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | ClientOptionsBody | Result<Unit> | E38 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `/clients/%7Binjected_client_id%7D/update` | `ClientApi.m16067postClientUpdategIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | ClientUpdatePostBody | Result<Unit> | E38 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v5/clients/%7Binjected_client_id%7D/client_verification/pin/resend` | `ClientApi.m16068resendClientVerificationCodeIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<ResendClientVerificationCodeResponse> | E38 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v5/clients/%7Binjected_client_id%7D/client_verification/pin/verify` | `ClientApi.m16069submitClientVerificationCodegIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | SubmitVerificationRequest | Result<PinVerificationResponse> | E38 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/clients/%7Binjected_client_id%7D/pin/verify` | `ClientApi.m16070verifyClientPINgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | VerifyPinBody | Result<PinVerificationResponse> | E38 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/clients/%7Binjected_client_id%7D/control_panel/delete` | `ClientDeviceManagementApi.m17603deleteClientgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | DeleteClientBody | Result<Unit> | E39 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/clients/%7Binjected_client_id%7D/control_panel/clients` | `ClientDeviceManagementApi.m17604getClientsIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<GetClientsResponse> | E39 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/clients/%7Binjected_client_id%7D/control_panel/request_pin` | `ClientDeviceManagementApi.m17605postManageClientsPinGenerateIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<GeneratePinResponse> | E39 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/clients/%7Binjected_client_id%7D/control_panel/pin/resend` | `ClientDeviceManagementApi.m17606postManageClientsPinResendIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<GeneratePinResponse> | E39 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/clients/%7Binjected_client_id%7D/control_panel/pin/verify` | `ClientDeviceManagementApi.m17607postManageClientsPinVerifygIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | VerifyPinPostBody | Result<VerifyPinResponse> | E39 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/commands/{command}` | `CommandApi.m16202commandPoll0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<SupervisorKommand> | E40 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/commands/{command}` | `CommandApi.m16203commandPollCameraAction0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CameraActionSupervisorKommand> | E40 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/commands/{command}` | `CommandApi.m16204commandPollLiveView0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<LiveViewSupervisorKommand> | E40 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/commands/{command}` | `CommandApi.m16205commandPollWithChildren0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<SupervisorKommandWithChildren> | E40 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/commands/{command}` | `CommandApi.commandPolling` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<Commands> | E40 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/commands/{command}/update` | `CommandApi.m16206postUpdateCommandBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateCommandRequest | Result<Kommand> | E40 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/commands/{command}/done` | `CommandApi.terminateCommand` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Call<BlinkData> | E40 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/commands/{command}/update` | `CommandApi.terminateOnboardingCommand` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | TerminateOnboardingBody | Observable<BlinkData> | E40 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v2/clients/%7Binjected_client_id%7D/tiv` | `CustomerSupportAccessApi.m17650postTivLockgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | TivLockBody | Result<SetTivLockResponse> | E41 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v2/clients/%7Binjected_client_id%7D/tiv_unlock/request_pin` | `CustomerSupportAccessApi.m17651postTivUnlockPinGenerateIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<GeneratePinResponse> | E41 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v2/clients/%7Binjected_client_id%7D/tiv_unlock/pin/resend` | `CustomerSupportAccessApi.m17652postTivUnlockPinResendIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<GeneratePinResponse> | E41 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v2/clients/%7Binjected_client_id%7D/tiv_unlock/pin/verify` | `CustomerSupportAccessApi.m17653postTivUnlockPinVerifygIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | VerifyPinPostBody | Result<VerifyPinResponse> | E41 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/devices/identify/{serialNumber}` | `DeviceApi.m16108getDeviceIdentitygIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<IdentifyDeviceResponse> | E42 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/devices/identify/{serialNumber}` | `DeviceApi.identifyDevice` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Single<IdentifyDeviceResponseOld> | E42 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/add` | `DoorbellApi.m16149addLotus0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AddLotusBody | Result<AddLotusResponse> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/doorbells/{doorbell_id}/change_wifi` | `DoorbellApi.m16150changeLotusWifiBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | OnboardingBody | Result<AddLotusResponse> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/doorbells/{lotusId}/delete` | `DoorbellApi.m16151deleteDoorbell0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/delete` | `DoorbellApi.deleteLotusRx` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<BlinkData> | E43 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/doorbells/{serial}/fw_update` | `DoorbellApi.downloadLotusFirmwareUpdate` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<retrofit2.adapter.rxjava.Result<ResponseBody>> | E43 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/doorbells/{doorbellId}/owl_as_chime/list` | `DoorbellApi.m16152getChimeCameras0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<ChimeCamerasResponse> | E43 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{doorbell}/config` | `DoorbellApi.m16153getDoorbellConfig0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<LotusConfigInfo> | E43 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{doorbell}/chime/{chimeType}/config` | `DoorbellApi.m16154getLotusChimeConfigBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<LotusChimeConfig> | E43 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{doorbell}/config` | `DoorbellApi.getLotusConfigRx` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<LotusConfigInfo> | E43 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/zones` | `DoorbellApi.getLotusZones` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<AdvancedCameraZones> | E43 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/zones` | `DoorbellApi.m16155getZonesV20E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<ZoneV2Response> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/doorbells/{doorbell_id}/stay_awake` | `DoorbellApi.m16156keepLotusAwake0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/doorbells/{doorbell_id}/change_mode` | `DoorbellApi.m16157lotusChangeMode0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<AddLotusResponse> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/doorbells/{doorbell_id}/clear_creds` | `DoorbellApi.m16158lotusClearCreds0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{doorbell}/power_test` | `DoorbellApi.m16159performLotusPowerAnalysis0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/doorbells/{doorbellId}/owl_as_chime/update` | `DoorbellApi.m16160postChimeCamerasBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | ChimeCamerasPostBody | Result<Unit> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/disable` | `DoorbellApi.m16161postDisableLotus0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CameraActionKommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/disable` | `DoorbellApi.postDisableLotusOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<Command> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/doorbells/{lotusId}/config` | `DoorbellApi.m16162postDoorbellConfigBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateLotusBody | Result<Kommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/doorbells/{lotusId}/status` | `DoorbellApi.m16163postDoorbellStatusCommand0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/enable` | `DoorbellApi.m16164postEnableLotus0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CameraActionKommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/enable` | `DoorbellApi.postEnableLotusOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<Command> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{networkId}/doorbells/{doorbellId}/liveview` | `DoorbellApi.m16165postLiveViewCommandBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | LiveViewCommandPostBody | Result<LiveViewCommandResponse> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{doorbellId}/temp_alert_disable` | `DoorbellApi.m16166postTempAlertDisable0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{doorbellId}/temp_alert_enable` | `DoorbellApi.m16167postTempAlertEnable0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{doorbellId}/calibrate` | `DoorbellApi.m16168postTemperatureCalibrationBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | TemperatureCalibrationPostBody | Result<Kommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/thumbnail` | `DoorbellApi.m16169postThumbnail0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CameraActionKommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/thumbnail` | `DoorbellApi.postThumbnailOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<Command> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/status` | `DoorbellApi.m16170refreshLotusStatus0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/status` | `DoorbellApi.m16171refreshLotusStatusSuspend0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Command> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/config` | `DoorbellApi.saveLotusSettings` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateLotusBody | Observable<Command> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{doorbell}/chime/{chimeType}/config` | `DoorbellApi.m16172setLotusChimeConfigyxL6bBk` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateLotusChimeConfig | Result<Kommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/zones` | `DoorbellApi.setLotusZones` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AdvancedCameraZones | Observable<Command> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{lotus}/zones` | `DoorbellApi.m16173setZonesV2BWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | ZoneV2Response | Result<Kommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/doorbells/{lotus_id}/snooze` | `DoorbellApi.m16174snoozeLotusBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | SnoozeBody | Result<Unit> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/doorbells/{doorbell}/trigger_chime` | `DoorbellApi.m16175testLotusDingBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | TestLotusDingConfig | Result<Kommand> | E43 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/doorbells/{lotus_id}/unsnooze` | `DoorbellApi.m16176unSnoozeLotus0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E43 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/clients/%7Binjected_client_id%7D/email_change` | `EmailChangeApi.m17629postEmailChangegIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | ChangeEmailPostBody | Result<Unit> | E44 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/clients/%7Binjected_client_id%7D/email_change/pin/resend` | `EmailChangeApi.m17630postEmailChangePinGenerateIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<GeneratePinResponse> | E44 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/clients/%7Binjected_client_id%7D/email_change/pin/verify` | `EmailChangeApi.m17631postEmailChangePinVerifygIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | VerifyPinPostBody | Result<VerifyPinResponse> | E44 |
...
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/feature_flags/enabled` | `FeatureFlagApi.m16211getFeatureFlagsIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<FeatureFlagsResponse> | E47 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v4/accounts/%7Binjected_account_id%7D/homescreen` | `HomeScreenApi.m17741getHomeScreenIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<HomeScreen> | E48 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/data_request/list` | `ManageDataApi.m17550getDataRequestsIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<DataRequests> | E50 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/data_request/dsar/create` | `ManageDataApi.m17551postDsarRequestIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<SubmitDataRequestResponse> | E50 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/data_request/euda/create` | `ManageDataApi.m17552postEudaRequestIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<SubmitDataRequestResponse> | E50 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/data_request/third_party/{thirdPartyId}/revoke` | `ManageDataApi.m17553postRevokegIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E50 |
| DELETE | https://rest-{shared_tier}.immedia-semi.com/api/ | `v4/accounts/%7Binjected_account_id%7D/media/{mediaId}/delete` | `MediaApi.m17899deleteCloudMediagIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E51 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/manifest/{manifestId}/clip/request/{clipId}` | `MediaApi.m17900getClipKommandyxL6bBk` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E51 |
| DELETE | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage` | `MediaApi.m17901getDeleteAllKommand0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E51 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/manifest/{manifestId}/clip/delete/{clipId}` | `MediaApi.m17902getDeleteLocalStorageMediaKommandyxL6bBk` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E51 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/manifest/request` | `MediaApi.m17903getLocalStorageManifestKommand0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E51 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/media/{commandId}` | `MediaApi.m17904getLocalStorageMediaBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<MediaResponse> | E51 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v4/accounts/%7Binjected_account_id%7D/media_settings` | `MediaApi.m17905getMediaSettingsIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<MediaSettingsResponse> | E51 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v4/accounts/%7Binjected_account_id%7D/unwatched_media` | `MediaApi.m17906getUnwatchedMediaIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<UnwatchedMediaResponse> | E51 |
| PATCH | https://rest-{shared_tier}.immedia-semi.com/api/ | `v4/accounts/%7Binjected_account_id%7D/media_settings` | `MediaApi.m17907patchMediaSettingsgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | MediaSettingsPatch | Result<Unit> | E51 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v4/accounts/%7Binjected_account_id%7D/media/delete` | `MediaApi.m17908postDeleteMediagIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | MediaListBody | Result<Unit> | E51 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v4/accounts/%7Binjected_account_id%7D/media/mark_all_as_viewed` | `MediaApi.m17909postMarkAllAsViewedIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E51 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v4/accounts/%7Binjected_account_id%7D/media/mark_as_viewed` | `MediaApi.m17910postMarkAsViewedgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | MediaListBody | Result<Unit> | E51 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v4/accounts/%7Binjected_account_id%7D/media` | `MediaApi.m17911postMediayxL6bBk` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | MediaPostBody | Result<MediaResponse> | E51 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/state/{type}` | `NetworkApi.armDisarmNetwork` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<Command> | E52 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/add` | `NetworkApi.m16506createSystemgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AddNetworkBody | Result<ANetwork> | E52 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/delete` | `NetworkApi.deleteSystem` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<BlinkData> | E52 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/state/disarm` | `NetworkApi.m16507disarmNetworkgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E52 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/system_offline/{network}` | `NetworkApi.sendSystemOfflineHelpEmail` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<BlinkData> | E52 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/snooze` | `NetworkApi.m16508snoozeSystem0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | SnoozeBody | Result<Unit> | E52 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/unsnooze` | `NetworkApi.m16509unSnoozeSystemgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E52 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/update` | `NetworkApi.updateNetworkSaveAllLiveViews` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateNetworkSaveAllLiveViews | Observable<BlinkData> | E52 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/update` | `NetworkApi.updateSystem` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateSystemNameBody | Observable<BlinkData> | E52 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{network}/update` | `NetworkApi.updateTimezone` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateTimezoneBody | Observable<BlinkData> | E52 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v2/notification` | `NotificationApi.acknowledgeNotification` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AcknowledgeNotificationBody | Observable<Object> | E53 |
...
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/accessories/rosie/owl/{owl_id}/calibrate` | `OwlApi.m16181calibrateRosie0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/change_wifi` | `OwlApi.m16182changeOwlWifiBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | OnboardingBody | Result<OwlAddBody> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/delete` | `OwlApi.m16183deleteOwl0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/delete` | `OwlApi.deleteOwlRx` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<BlinkData> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/owls/{owl_id}/accessories/rosie/{rosie_id}/delete` | `OwlApi.m16184deleteRosieBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E55 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/owls/{serial}/fw_update` | `OwlApi.downloadOwlFirmwareUpdate` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<retrofit2.adapter.rxjava.Result<ResponseBody>> | E55 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/config` | `OwlApi.m16185getOwlConfig0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<OwlConfigInfo> | E55 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/config` | `OwlApi.getOwlConfigRx` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<OwlConfigInfo> | E55 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/zones` | `OwlApi.m16186getZonesV20E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<ZoneV2Response> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/owls/add` | `OwlApi.m16187postAddOwl0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AddOwlPostBody | Result<AddOwlResponse> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/change_wifi` | `OwlApi.m16188postChangeOwlWifiBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AddOwlPostBody | Result<AddOwlResponse> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{camera}/lights/{lightControl}` | `OwlApi.m16189postLightBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CameraActionKommand> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{camera}/lights/{lightControl}` | `OwlApi.m16190postLightOldBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/liveview` | `OwlApi.m16191postLiveViewCommandBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | LiveViewCommandPostBody | Result<LiveViewCommandResponse> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/config` | `OwlApi.m16192postOwlConfigBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateOwlBody | Result<Kommand> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/config` | `OwlApi.m16193postOwlConfigCommandBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateOwlBody | Result<CameraActionKommand> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/config` | `OwlApi.postOwlSettingsOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateOwlBody | Observable<Command> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/status` | `OwlApi.m16194postOwlStatusCommand0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/thumbnail` | `OwlApi.m16195postThumbnail0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<CameraActionKommand> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/thumbnail` | `OwlApi.postThumbnailOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<Command> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{networkId}/owls/{owlId}/zones` | `OwlApi.m16196setZonesV2BWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | ZoneV2Response | Result<Kommand> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/owls/{owl_id}/snooze` | `OwlApi.m16197snoozeOwlBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | SnoozeBody | Result<Unit> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/owls/add` | `OwlApi.m16198startOwlOnboardingOld0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | OnboardingBody | Result<OwlAddBody> | E55 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network_id}/owls/{owl_id}/unsnooze` | `OwlApi.m16199unSnoozeOwl0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E55 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/clients/%7Binjected_client_id%7D/password_change` | `PasswordChangeApi.m17642changePasswordgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | ResetPasswordPostBody | Result<Unit> | E56 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/clients/%7Binjected_client_id%7D/password_change` | `PasswordChangeApi.m17643postPasswordChangegIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | ResetPasswordPostBody | Result<Unit> | E56 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/clients/%7Binjected_client_id%7D/password_change/pin/generate` | `PasswordChangeApi.m17644postPasswordChangePinGenerateIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<GeneratePinResponse> | E56 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v4/clients/%7Binjected_client_id%7D/password_change/pin/verify` | `PasswordChangeApi.m17645postPasswordChangePinVerifygIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | VerifyPinPostBody | Result<VerifyPinResponse> | E56 |
...
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v5/clients/%7Binjected_client_id%7D/phone_number_change` | `PhoneNumberChangeApi.m16076changePhoneNumbergIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | ChangePhoneNumberBody | Result<ChangePhoneNumberResponse> | E58 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v5/clients/%7Binjected_client_id%7D/phone_number_change` | `PhoneNumberChangeApi.m16077postPhoneNumberChangegIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AddPhoneNumberPostBody | Result<ChangePhoneNumberResponse> | E58 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v5/clients/%7Binjected_client_id%7D/phone_number_change/pin/verify` | `PhoneNumberChangeApi.m16078postPhoneNumberChangePinVerifygIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | SubmitVerificationRequest | Result<PinVerificationResponse> | E58 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/programs/create` | `ProgramApi.createProgram` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | Program | Observable<BlinkData> | E59 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/programs/{program}/delete` | `ProgramApi.deleteProgram` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<BlinkData> | E59 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/programs/{program}/disable` | `ProgramApi.disableProgram` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<BlinkData> | E59 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/programs/{program}/enable` | `ProgramApi.enableProgram` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<BlinkData> | E59 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/programs` | `ProgramApi.getPrograms` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<List<Program>> | E59 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{network}/programs/{program}/update` | `ProgramApi.updateProgram` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | UpdateProgramRequest | Observable<BlinkData> | E59 |
...
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/access` | `ReadSubscriptionApi.m16219getAccessIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<AccessResponse> | E61 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/subscriptions/entitlements` | `ReadSubscriptionApi.m16220getEntitlementsIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<EntitlementResponse> | E61 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v3/accounts/%7Binjected_account_id%7D/subscriptions/plans` | `ReadSubscriptionApi.m16221getSubscriptionsIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<SubscriptionPlansResponse> | E61 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/subscriptions/plans` | `ReadSubscriptionApi.m16222getSubscriptionsOldIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<SubscriptionPlansResponse> | E61 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/smart_video_descriptions` | `SmartVideoDescriptionsApi.m17330getSmartVideoDescriptionsIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<SmartVideoDescriptionsResponse> | E63 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/smart_video_descriptions` | `SmartVideoDescriptionsApi.m17331postUpdateSmartVideoDescriptionsgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | SmartVideoDescriptionsPostBody | Result<Unit> | E63 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/delete` | `SyncModuleApi.m17015deleteSyncModule0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E64 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/delete` | `SyncModuleApi.deleteSyncModuleRx` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<BlinkData> | E64 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/sync_modules/{serial}/fw_update` | `SyncModuleApi.downloadFirmwareUpdate` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Observable<retrofit2.adapter.rxjava.Result<ResponseBody>> | E64 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/eject` | `SyncModuleApi.m17016ejectUsbStorage0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E64 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/eject` | `SyncModuleApi.ejectUsbStorageOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Single<Command> | E64 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/format` | `SyncModuleApi.m17017formatUsbStorage0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E64 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/format` | `SyncModuleApi.formatUsbStorageOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Single<Command> | E64 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/status` | `SyncModuleApi.m17018getLocalStorageStatus0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<LocalStorageStatusResponse> | E64 |
| GET | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/status` | `SyncModuleApi.getLocalStorageStatusOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Single<LocalStorageStatusResponse> | E64 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/mount` | `SyncModuleApi.m17019mountUsb0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Kommand> | E64 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v1/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{syncModuleId}/local_storage/mount` | `SyncModuleApi.mountUsbOld` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Single<Command> | E64 |
| POST | https://rest-{shared_tier}.immedia-semi.com/api/ | `v2/accounts/%7Binjected_account_id%7D/networks/{networkId}/sync_modules/{type}` | `SyncModuleApi.m17020startSyncModuleOnboardingBWLJW6A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | OnboardingBody | Result<Command> | E64 |
...
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/subscriptions/plans/{subscriptionId}/attach` | `WriteSubscriptionApi.m16239attachPlan0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AttachPlanBody | Result<DspSubscriptionResponse> | E72 |
| DELETE | https://rest-{tier}.immedia-semi.com/api/ | `v1/subscriptions/plans/cancel_trial` | `WriteSubscriptionApi.m16240cancelTrialIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E72 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v2/subscriptions/plans/create_trial` | `WriteSubscriptionApi.m16241createTrialgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | AdditionalTrialBody | Result<Unit> | E72 |
| GET | https://rest-{tier}.immedia-semi.com/api/ | `v1/subscriptions/plans/get_device_attach_eligibility` | `WriteSubscriptionApi.m16242getDeviceEligibilityIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<DeviceEligibilityResponse> | E72 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/subscriptions/link/link_account` | `WriteSubscriptionApi.m16243linkAmazonAccountgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | MapLinkBody | Result<DspSubscriptionResponse> | E72 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/subscriptions/clear_popup/{type}` | `WriteSubscriptionApi.m16244postClearTrialPopupgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E72 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/subscriptions/plans/renew_trial` | `WriteSubscriptionApi.m16245renewTrialIoAF18A` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | — | Result<Unit> | E72 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/subscriptions/request/status/{uuid}` | `WriteSubscriptionApi.m16246subscriptionRequestStatus0E7RQCE` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | SubscriptionRequestStatusBody | Result<SubscriptionRequestStatusResponse> | E72 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/subscriptions/link/unlink_account` | `WriteSubscriptionApi.m16247unlinkAmazonAccountgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | VerifyLinkAccountBody | Result<DspSubscriptionResponse> | E72 |
| GET | dynamic (@Url) | `@Url (full URL)` | `VideoApi.m17790getVideogIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists (Blink hosts only) | default (+ Authorization/TOKEN-AUTH if host is immedia-semi) | — | Result<ResponseBody> | E66 |
...
| Method | Base Host | Path | Purpose (method) | Auth | Headers | Body Schema | Response Shape | Evidence |
|---|---|---|---|---|---|---|---|---|
| POST | https://rest-{tier}.immedia-semi.com/api/ | `v1/events/app` | `EventApi.m16255trackEventsgIAlus` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | TrackingEvents | Result<Unit> | E45 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `/app/logs/upload` | `LogApi.sendLogs` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | LogsBody | Observable<BlinkData> | E49 |
| POST | https://rest-{tier}.immedia-semi.com/api/ | `/app/logs/upload` | `LogApi.sendLogsCall` | Bearer; optional `TOKEN-AUTH` when registration-token state exists | default | LogsBody | Call<BlinkData> | E49 |
...
- Find where `SHARED_TIER` is persisted (setter/location not found yet).
